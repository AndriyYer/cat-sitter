...
function App() {
  ...
  const [currentView, setCurrentView] = useState<"schedule" | "calendar">("schedule"); // Set default to 'schedule'
  ...
  }, [selectedDate, claimedDates, fetchBookingDetails]);
  ...
  const toggleView = () => {
    setCurrentView((prev) => (prev === "schedule" ? "calendar" : "schedule"));
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-50 px-4 py-8">
      ...
      <p className="text-lg text-gray-600 mb-8">View all bookings below</p>
      ...
      <div className="mb-4">
        ...
        >
          Switch to {currentView === "schedule" ? "Calendar" : "Schedule"} View
        </button>
      </div>
